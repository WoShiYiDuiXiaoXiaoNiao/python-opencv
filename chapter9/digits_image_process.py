...
ann, test_data = ANN.train(ANN.create_ANN(58), 50000, 2)
...
for c in cntrs:
  ...
  b_area = (img.shape[0]-3) * (img.shape[1] - 3)
  ...
  for q in rectangles:
    if inside(c_rect, q):
      ...
      break
  if not is_inside:
    if not c_area == b_area:     #判断是不是将整个图片轮廓作为元素添加进来，
      rectangles.append(c_rect)  #如果没有这一步，是的话整个数组不会再添加元素
